#!/usr/bin/python2.6
# -*- coding: utf-8 -*-
''' Prepares htseq-count
Takes an infolder containing folders of sam files and prepares .sh scripts to run htseq-count. 
...
parser.add_argument("infolder", type=str,
                    help="Infolder of sorted sam files on which to run htseq-count")
parser.add_argument("wdpath", type=str,
                    help="Working directory path")
parser.add_argument("gtfpath", type=str,
                    help="Path and name of reference GTF file")
...
def list_files(current_dir):
    ...
    for path, subdirs, files in os.walk(current_dir): # Walk directory tree
        for name in files:
            if name.endswith("name_sorted.sam"):
...
def main():
    ...
    script_folder_path = wdpath+"/HTSeq-count_scripts"
    ...
    for infolder in infolders:
        ...
        if len(infile) == 1:
            ...
            scriptname = script_folder_path+"/"+sample_id+"_htseqcount.sh"
            htseqout = wdpath+"/"+os.path.basename(infile).split(".bam")[0]+"_htseqcount.txt"
        ...
        with open(scriptname, "w") as outfile:
            ...
            outfile.write("\n")
            outfile.write("\n")
            outfile.write("source activate htseq")
            outfile.write("\n")
            outfile.write("\n")

            outfile.write("htseq-count -f sam -r name -s no")
            outfile.write(" ")
            outfile.write(str(infile))
            outfile.write(" ")
            outfile.write(args.gtfpath)
            outfile.write(" ")
            outfile.write(">")
            outfile.write(" ")
            outfile.write(htseqout)
            outfile.write("\n")
            outfile.write("\n")
